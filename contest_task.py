...
def fetch_nk(lim:int):
    ...
    for h4 in soup.find_all('h4'):
        ...
        name=re.split(r'原创',txt)[0]; name=re.sub(r'(报名中|距比赛.*)$','',name).strip('· ').strip()
        ...
        if not m: continue
        try: dt=datetime.strptime(m.group(1),'%Y-%m-%d %H:%M').replace(tzinfo=TZ_CN)
        except: continue
        if dt<now: continue
        res.append({'name':name,'start':dt})
        if len(res)>=lim: break
...
def fetch_ac(lim:int):
    ...
    for row in table.find('tbody').find_all('tr'):
        ...
        try: dt=datetime.strptime(cols[0].get_text(strip=True),'%Y-%m-%d %H:%M:%S%z').astimezone(timezone.utc)
        except: continue
...
def fetch_lg(lim:int):
    ...
    res=[]
    for c in raw:
        name=(c.get('name') or '').strip(); ts=c.get('startTime')
        ...
        if dt<now: continue
        res.append({'name':name,'start':dt})
    res.sort(key=lambda x:x['start']); return res[:lim]
...
def get_contests():
    ...
    for site,fetcher,key in FETCHERS:
        try:data=fetcher(LIMITS[key])
        except Exception as e:
            print(f'[{site}] 抓取失败：{e}', file=sys.stderr); data=[]
        for d in data: contests.append({'site':site,'name':d['name'],'start':d['start']})
    contests.sort(key=lambda x:x['start'])
    return contests

# ---------- 主程序 ----------
if __name__=='__main__':
    if str(OUTPUT_FILE).startswith('/absolute/path'):
        print('❗ 请先在 contest_task.py 中修改 OUTPUT_FILE 为实际绝对路径')
        sys.exit(1)
